...
import type {Account, Category, CategoryAmount, Expense} from "../../lib/types.ts";
...
// Ustawienie lokalizacji dla dayjs na polski
dayjs.locale('pl');
    ...
    const processWeeklyData = (expenses: Expense[], startDateStr: string) => {
        ...
        expenses.forEach(exp => {
            ...
            if (daysMap.has(dateStr)) {
                daysMap.set(dateStr, (daysMap.get(dateStr) || 0) + Math.abs(exp.price));
            }
        });
                        ...
                        >
                            <CardContent>
                                <Stack direction="row" justifyContent="space-between" alignItems="start">
                                    ...
                                    <AccountBalanceWalletIcon sx={{ color: hexToRgba(account.color || '#000', 0.3), fontSize: 32 }} />
                                </Stack>
                            </CardContent>
                        </Card>
                    </Grid>
                ...
                <Grid item xs={12} md={5} sx={{flex: 1}}>
                    <Card flex={1} variant="outlined" sx={{ borderRadius: 2, height: '100%', display: 'flex', flexDirection: 'column' }}>
                        <CardContent sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                            ...
                            <Box sx={{ flex: 1, minHeight: 250, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                    ...
                                    {categoryAmounts.slice(0, 5).map((cat) => {
                                        ...
                                        return (
                                            <Box key={cat.category} display="flex" justifyContent="space-between" alignItems="center">
                                                ...
                                                <Typography variant="body2" fontWeight="bold">{formatMoney(cat.amount)}</Typography>
                                            </Box>
                                        )
                                    })}
                                </Stack>
                            ...
                            return (
                                <React.Fragment key={expense.id}>
                                    <ListItem sx={{ px: 1 }}>
                                        ...
                                        <ListItemText
                                            primary={
                                                <Typography fontWeight="medium">{expense.description || 'Bez opisu'}</Typography>
                                            }
                                            secondary={
